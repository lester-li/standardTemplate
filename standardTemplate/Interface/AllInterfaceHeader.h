//
//  AllInterfaceHeader.h
//  RepairService_Mechanic
//
//  Created by macbook on 16/9/12.
//  Copyright © 2016年 macbook. All rights reserved.
//
#ifdef __OBJC__
#ifndef AllInterfaceHeader_h
#define AllInterfaceHeader_h

// 通用 头文件集合
#import "AppDelegate.h"                 // app 代理              // 通用宏定义
//#import "MJRefresh.h"                 // 刷新
#import "UIImageView+WebCache.h"        // 图片加载
//#import "MBProgressHUD+MJ.h"
#import "NSString+MD5Str.h"
#import "UIImage+Common4IOS.h"
#import "MBProgressHUD.h"               // 加载提示框
//#import "QuickControl.h"
//#import "Masonry.h"
#import "MJExtension.h"
#import "UIView+Extension.h"
//#import "HttpRequest.h"
#import "MJRefresh.h"
//#import "MDateTool.h"
#import "AFNetworking.h"
#import "UIColor+Common4IOS.h"
#import "XBNetAcess.h"
#import "UIGestureRecognizer+YYAdd.h"
#import "NSDictionary+Extension.h"
#import "NSString+Frame.h"
#import "UIButton+title_imageView_layout.h"
#import "UILabel+align.h"
//#import "NSString+UUID.h"
//#import "UserModel.h"
#import "UIButton+WebCache.h"
//#import <Hyphenate_CN/EMSDKFull.h>
//#import "VoiceConverter.h"
#import "SVProgressHUD.h"
#import "OtherInterface.h"
#import "QuickControl.h"



/**
 *  项目头文件
 */
// tabbar
//#import "HKTabBarViewController.h"       
// 轮播图
#import "SDCycleScrollView.h"
// tableview
#import "Main_TabelView.h"

//#import "EaseUI.h"

#endif /* AllInterfaceHeader_h */
#endif
